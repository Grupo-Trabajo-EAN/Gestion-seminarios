...
router.get('/', (req, res) => {
  ...
  const query = `
    SELECT 
      ...
      pa.fecha_aprobacion,
      (SELECT 
        GROUP_CONCAT(CONCAT(p.nombre, ' ', p.apellido, ' (', sp.rol, ')') SEPARATOR '; ') 
       FROM semillero_profesores sp
       JOIN profesores p ON sp.profesor_id = p.id
       WHERE sp.semillero_id = pa.Semillero AND sp.estado = 'activo'
      ) AS aprobadores
...
router.get('/:planId/aprobadores', (req, res) => {
    ...
    db.query(query, [planId], (err, results) => {
        if (err) return res.status(500).json({ error: 'Error al buscar aprobadores' });
        res.json(results);
    });
});
